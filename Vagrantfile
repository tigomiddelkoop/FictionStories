...
Vagrant.configure("2") do |config|
  ...
  config.vm.provider "virtualbox" do |v|
    v.cpus = 4
    v.memory = 4096
    v.name = "FictionStories"
    v.customize ["setextradata", :id, "VBoxInternal2/SharedFoldersEnableSymlinksCreate/v-root", "1"]
  end


  if OS.mac?
    config.vm.synced_folder ".", "/var/www/fictionstories"
  else
    config.vm.synced_folder ".", "/var/www/fictionstories", :type => "nfs", :nfs_version => 4
  end



  config.vm.network "private_network", :ip => '192.168.10.200', :adapter => 2
